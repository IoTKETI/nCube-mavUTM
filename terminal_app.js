...
function actionAllGoto(input) {
    ...
    for (var idx in cur_drone_list_selected) {
        if (cur_drone_list_selected.hasOwnProperty(idx)) {
            var drone_selected = cur_drone_list_selected[idx].name;
            cur_goto_position = arr_goto_all_position[goto_all_index[cur_drone_list_selected[idx].name]];
            command_delay++;
            ...
            goto_position_selected[idx] = cur_goto_position;
...
function allHoldMenu() {
    ...
    for (idx in cur_drone_list_selected) {
        if (cur_drone_list_selected.hasOwnProperty(idx)) {
            ...
            setTimeout(send_goto_command, 10 * command_delay, drone_selected, target_pub_topic[drone_selected], target_system_id[drone_selected], lat, lon, alt);
        }
    }
...
function actionAllProgressBar(selectedIndex, input) {
    ...
    doAllProgress(selectedIndex, input, function (code) {
        if (code === '404') {
            ...
            term.moveTo.eraseDisplayBelow.red(1, conf.drone.length + 4, 'Drone is no response\n');
            setTimeout(allMenu, back_menu_delay + back_menu_delay * (cur_drone_list_selected.length + 1));
        }
        else {
...
function actionAllAutoGoto(selectedGotoIndex) {
    ...
    if (input.split('|')[0] === 'cancel') {
        ...
        setTimeout(actionAllProgressBar, back_menu_delay + back_menu_delay * (cur_drone_list_selected.length + 1), selectedGotoIndex, input);
    }
}
                ...
                if (cur_drone_list_selected.hasOwnProperty(idx)) {
                    ...
                    if (hb.hasOwnProperty(target_system_id[drone_selected])) {
                        if (hb[target_system_id[drone_selected]].base_mode & 0x80) {
                            setTimeout(send_start_mission_command, back_menu_delay * command_delay, drone_selected, target_pub_topic[drone_selected], target_system_id[drone_selected], start_number, 10);
                        }
                        else {
                            setTimeout(send_arm_command, back_menu_delay * command_delay, drone_selected, target_pub_topic[drone_selected], target_system_id[drone_selected], 1, 0);

                            setTimeout(send_start_mission_command, back_menu_delay * 60 + back_menu_delay * command_delay, drone_selected, target_pub_topic[drone_selected], target_system_id[drone_selected], start_number, 10);
                        }
                    }
                }
            }
...
function send_arm_command(target_name, pub_topic, target_sys_id, param1, param2) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Arm command to %s\n', target_name);
...
function send_start_mission_command(target_name, pub_topic, target_sys_id, param1, param2) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Start Mission command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
function send_set_mode_command(target_name, pub_topic, target_sys_id, base_mode, custom_mode) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Set Mode command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
function send_change_speed_command(target_name, pub_topic, target_sys_id, target_speed) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Change_Speed command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
function send_set_roi_command(target_name, pub_topic, target_sys_id, target_lat, target_lon, target_alt) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Set ROI command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
function send_set_servo_command(target_name, pub_topic, target_sys_id, target_number, target_pwm) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Set Servo command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
function send_set_relay_command(target_name, pub_topic, target_sys_id, target_number, target_val) {
    ...
    try {
        ...
        if (msg == null) {
            ...
            term.moveTo.blue(1, conf.drone.length + column_count++, 'Send Set Relay command to %s\n', target_name);
            term.moveTo.red(1, conf.drone.length + column_count++, 'msg: ' + msg.toString('hex') + '\n');
            mqtt_client.publish(pub_topic, msg);
        }
    }
...
//                if (placeFlag === 'allGotoMenu') {
                    var arr_cur_goto_position = goto_position_selected[idx].split(':');
                    var tar_lat = parseFloat(arr_cur_goto_position[0]);
                    var tar_lon = parseFloat(arr_cur_goto_position[1]);
                    var tar_alt = parseFloat(arr_cur_goto_position[2]);
